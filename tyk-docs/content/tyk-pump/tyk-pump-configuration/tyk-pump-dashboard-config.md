...
To enable [Dashboard Analytics]({{<ref "tyk-dashboard-analytics">}}), configure Tyk Pump to send analytic data to the Dashboard datastore MongoDB / SQL.

{{< tabs_start >}}
{{< tab_start "MongoDB" >}}

## Different Types of Mongo Pumps

Tyk offers 3 types of Mongo pumps:

1. [mongo-pump-aggregate](#mongo-pump-aggregate) - **`mongo-pump-aggregate`** pump stores data in a collection called **z_tyk_analyticz_aggregate_{ORG ID}**. 
The Tyk Dashboard uses the `mongo-pump-aggregate` collection to display analytics. This is different from the standard `mongo` pump plugin that stores individual analytic items into MongoDB. The aggregate functionality was built to be fast, as querying raw analytics is expensive in large data sets.

2. [mongo](#mongo-pump) - **`mongo`** Pump simply saves all individual requests across every organisation to a collection called **`tyk_analytics`**. Each request will be stored as a single document.

3. [mongo-pump-selective](#mongo-pump-selective) - **`mongo-pump-selective`** pump stores individual requests per organisation in collections called **`z_tyk_analyticz_{ORG ID}`**. Similar to the regular `mongo` pump, Each request will be stored as a single document.
...
### Mongo Selective Pump Configuration
...
## Different Types of SQL Pumps

TODO When using one of our [supported SQL platforms]({{< ref "/content/tyk-dashboard/database-options.md#introduction" >}}), Tyk offers 3 types of SQL pumps:
...
TODO Is it available for Mongo?

### SQL Uptime Pump Configuration

In an `uptime_pump_config` section, you can configure a SQL uptime pump. To do that, you need to add the field `uptime_type` with `sql` value. It will store logs into the `tyk_aggregated` database table.
