...
Additionally you can now mix policies defined for the same API but having different path and methods access rules. For example you can have one policy which allows only access to `/users`  and a second policy giving user access to a `/companies` path. If you create a key with both policies, their access rules will be merged, and user will get access to both paths.
...
WSDL now is a first class citizen at Tyk. You can take your WSDL definition and simply import to the Dashboard, creating a nice boilerplate for your service. Additionally we have documentation on how to work with SOAP of any complexity with Tyk. See our [SOAP documentation](/docs/concepts/soap) for more details.
