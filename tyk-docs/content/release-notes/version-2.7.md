...
From an API standpoint, user groups can be managed by [new Dashboard API][7]. The User object now has a new `group_id` field, and if it is specified, all permissions will be inherited from the specified group. [SSO API][6] has been updated to include `group_id` field as well.
