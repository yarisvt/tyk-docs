...
cat /tmp/urlcheck.new.json | jq -r '.path' | sed -E "s|/nightly||g" | sed -E "s|/docs/docs||" | sed -E "s|/docs||" | sed -E "s|^/||" | sed -E "s|/$||" | sort | uniq > /tmp/urlcheck.new
cat /tmp/urlcheck.prev.json | jq -r '.path' | sed -E "s|/nightly||g" | sed -E "s|/docs/docs||" | sed -E "s|/docs||" | sed -E "s|^/||" | sed -E "s|/$||" | sort | uniq > /tmp/urlcheck.prev
